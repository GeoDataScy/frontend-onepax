...
export function BoardingFormCatraca1({
  ...
  const handleHabilitar = async () => {
    try {
      ...
      setIsEnabled(true);
      toast.success("Catraca 1 habilitada — contador zerado!");
    } catch (error) {
      toast.error("Erro ao habilitar Catraca 1");
    }
  };
  ...
  const handleLiberar = async () => {
    try {
      await catraca1Service.liberarPassageiro();
      toast.success("Comando enviado para a Catraca 1!");
    } catch (error) {
      toast.error("Falha ao enviar comando para a Catraca 1.");
    }
  };
